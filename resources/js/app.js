...
(function () {
	...
	function initActiveMenu() {
		...
		if (baseUrl) {
			// navbar-nav
			var a = document.getElementById("navbar-nav").querySelector('[href="' + baseUrl + '"]');
			//var a = document.getElementById("navbar-nav").querySelector('.nav-link');
			//console.log(a);
			if (a) {
				a.classList.add("active");
				var parentCollapseDiv = a.closest(".collapse.menu-dropdown");
	...
	// function initActiveMenu() {
	// 	// Mendapatkan URL lengkap saat ini
	// 	var currentUrl = window.location.href;
	
	// 	// Navbar-nav
	// 	var links = document.getElementById("navbar-nav").querySelectorAll('.nav-link');
	
	// 	links.forEach(link => {
	// 		// Jika href link cocok dengan URL saat ini
	// 		if (link.getAttribute('href') === currentUrl) {
	// 			// Tambahkan class 'active' ke elemen link tersebut
					...
					if (elem.querySelectorAll(".notification-item").length > 0) {
						...
						if (!emptyNotificationElem) {
							...
							<div class="w-25 w-sm-50 pt-3 mx-auto">\
								<img src="'+window.location.origin+'/build/images/svg/bell.svg" class="img-fluid" alt="user-pic">\
							</div>\
