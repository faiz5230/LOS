...
class DataDebiturUMKM implements FromView, WithStyles, WithDrawings 
{
    protected $id;
    ...
    {
        $debitur = DebiturUMKM::with('simulation')->findOrFail($this->id);
        
        return view('exports.data_debitur_umkm', [
            'debitur' => $debitur
        ]);
    }
    ...
    public function styles(Worksheet $sheet)
    {
        ...
        for ($row = 1; $row <= $highestRow; $row++) {
            ...
            if (in_array($cellValue, ['DATA DEBITUR UMKM', 'DATA SAUDARA YANG TIDAK SERUMAH', 'PERMOHONAN KREDIT'])) {
                $dataRows[] = $row;
